...
var menuData = '<a id="logBtn"><i class="fas fa-clipboard fa-fw"></i> 开启日志</a>' +
  '<a id="downloadLogBtn"><i class="fas fa-download fa-fw"></i> 下载日志</a>'
...
module.exports = function socket (socket) {
  ...
  conn.on('ready', function connOnReady () {
    ...
    socket.emit('footer', socket.request.session.username + '@' + socket.request.session.ssh.host + ':' + socket.request.session.ssh.port)
    socket.emit('status', '***SSH 连接成功,开始你的表演***')
  ...
  if (socket.request.session.username && socket.request.session.userpassword && socket.request.session.ssh) {
    ...
    conn.connect({
      ...
      username: 'root',
      privateKey: fs.readFileSync('/Users/ahaii/.ssh/id_rsa'),
